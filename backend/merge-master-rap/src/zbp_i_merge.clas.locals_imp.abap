CLASS lhc_Merge DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    METHODS validateMergeQty FOR VALIDATE ON SAVE
      IMPORTING keys FOR Merge~validateMergeQty.
ENDCLASS.
...
CLASS lhc_Merge IMPLEMENTATION.
  ...
  METHOD validateMergeQty.
    READ ENTITIES OF zi_merge IN LOCAL MODE
      ENTITY Merge FIELDS ( Quantity ) WITH CORRESPONDING #( keys )
      RESULT DATA(rows).
    LOOP AT rows INTO DATA(row).
      IF row-Quantity <= 0.
        APPEND VALUE #( %tky = row-%tky ) TO failed-merge.
        APPEND VALUE #( %tky = row-%tky
                        ...
                        %element-Quantity = if_abap_behv=>mk-on ) TO reported-merge.
      ENDIF.
    ENDLOOP.
  ENDMETHOD.
