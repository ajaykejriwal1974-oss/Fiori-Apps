# HU Physical Inventory (ZHUINV) — custom Fiori app

Freestyle SAPUI5 worklist app, the clean-core replacement for **ZSOL_MTOS_PROCESS (phys. inv. document create)**.
Binds to the OData V4 service of [`backend/hu-phys-inventory-rap`](../../backend/hu-phys-inventory-rap)
(`HuPhysInv` entity set) and exposes the service's static action(s) as
table actions.

## Worklist columns
`HandlingUnit`, `Reference`, `PackagingMaterial`
...
- `REPLACE_WITH_HU_PHYS_INV_SERVICE` (manifest.json) → the OData V4 service binding
  name created in ADT for service definition `ZUI_HU_PHYS_INV` (e.g. `ZUI_HU_PHYS_INV_O4`).
