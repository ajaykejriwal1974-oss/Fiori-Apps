...
## Worklist columns
`Material`, `Plant`, `Batch`, `LastGoodsReceiptDate`
...
- `REPLACE_WITH_BATCH_STATUS_SERVICE` (manifest.json) → the OData V4 service binding
  name created in ADT for service definition `ZUI_BATCH_STATUS` (e.g. `ZUI_BATCH_STATUS_O4`).
