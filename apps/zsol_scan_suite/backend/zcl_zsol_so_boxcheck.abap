...
*
* THE SINGLE BATCH ON THE ITEM (07.09.2026, v5)
*
* An item carries EITHER one batch in VBAP-CHARG OR a list in
* ZVBAP_BATCH - MV45AFZZ refuses both ("Single Batch And Multiple
* Batches are not Allowed at Single line item"). v4 therefore refused to
* touch an item with a single batch and sent the operator to VA02. In
* production (plant 2002, 07.09) that was every item on every order: the
* dyeing plant's orders name one lot per item, the lot is sold out and
* the next lot has to go on the item - which is exactly the change the
* card was built to make. So, for an item with a single batch:
*
*   ASSIGN_BATCH  replaces VBAP-CHARG with the batch given, through
*                 BAPI_SALESORDER_CHANGE (the same document change VA02
*                 makes, with the same checks - a delivered item, a
*                 locked order, a credit block all come back as SAP's
*                 own message). Logged with action 'C' and the old batch
*                 in NOTE.
*   REMOVE_BATCH  of that batch clears VBAP-CHARG the same way, logged
*                 'R' - the item then names no batch, and a list can be
*                 built on it with ASSIGN_BATCH as before.
*
* The list path (ZVBAP_BATCH rows) is unchanged. Both paths still demand
* that the batch be packed stock of the item's material at its plant.
* ----------------------------------------------------------------------
  ...
  PUBLIC SECTION.
    ...
    " Name a batch on an item. On an item with a batch list (or none) this
    " is the ZVBAP_BATCH row the VA02 item screen would write; on an item
    " with a single batch in VBAP-CHARG it REPLACES that batch (v5).
    ...
    " Take a batch off an item: the ZVBAP_BATCH row, or - when the batch
    " is the item's single VBAP-CHARG - that field, cleared (v5).
  ...
  PRIVATE SECTION.
    ...
    " Is this batch packed, in-stock HU of the material at the plant?
    CLASS-METHODS is_packed_stock
      IMPORTING
        !iv_matnr        TYPE matnr
        !iv_werks        TYPE werks_d
        !iv_charg        TYPE charg_d
    ...
    " VBAP-CHARG of one item set to IV_CHARG (space clears it) through
    " BAPI_SALESORDER_CHANGE. Returns SAP's first error message, or blank
    " when the change is in the update task - the caller commits.
    CLASS-METHODS set_item_batch
      IMPORTING
        !iv_vbeln       TYPE vbeln_va
        !iv_posnr       TYPE posnr_va
        !iv_charg       TYPE charg_d
...
CLASS zcl_zsol_so_boxcheck IMPLEMENTATION.
  ...
  METHOD check_order.
    ...
    LOOP AT rt_items ASSIGNING FIELD-SYMBOL(<ls_item>).
      ...
      " The batches this item names: its ZVBAP_BATCH rows, else its
      " VBAP-CHARG - per item, the same way ZRPT_SALES_SCAN_FR reads them
      " since KSDK906923 (before that the report keyed the ZVBAP_BATCH
      " read on the order and pinned every batch to the first item).
      CLEAR lt_item_b.
      ...
      ELSEIF lt_c_batch IS NOT INITIAL.
        ...
        ELSEIF <ls_item>-batch_src = 'C'.
          " A single batch on the item itself (VBAP-CHARG). Since v5 the
          " card replaces it with the tapped lot (ASSIGN_BATCH).
          <ls_item>-hint = |No packed stock carries the item's batch { <ls_item>-charg } - put one of the lots listed on the item in its place|.
        ELSE.
  ...
  METHOD write_log.
    ...
    " A = assigned (list row), R = removed (list row or single batch
    " cleared), C = single batch on the item changed (v5; NOTE says from
    " which). SOURCE says which program wrote the row; today only the Scan
    " Suite does ('SCAN'), VA02's batch tab does not log, so absence of a
    " row means "changed in SAP directly".
  ...
  METHOD set_item_batch.

    " The document change VA02 makes when the Batch field of the item is
    " overwritten (or emptied): BAPI_SALESORDER_CHANGE with the one field
    " flagged. Everything SAP checks on that screen - the item already
    " delivered, the order open in VA02 by somebody, a credit block, the
    " customer's own MV45AFZZ rules - comes back in RETURN and is handed
    " to the operator word for word. Nothing is committed here.
    ...
    APPEND VALUE #( itm_number = iv_posnr batch = iv_charg ) TO lt_item.
    APPEND VALUE #( itm_number = iv_posnr updateflag = 'U' batch = abap_true ) TO lt_itemx.
  ...
  METHOD assign_batch.
    ...
    SELECT SINGLE matnr, werks, abgru, charg FROM vbap
      ...
      INTO @DATA(ls_item).
    IF sy-subrc <> 0.
      ev_error = |Item { CONV i( iv_posnr ) } of order { iv_vbeln ALPHA = OUT } does not exist|.
      RETURN.
    ENDIF.
    IF ls_item-abgru IS NOT INITIAL.
      ev_error = |Item { CONV i( iv_posnr ) } is rejected|.
      RETURN.
    ENDIF.
    ...
    IF is_packed_stock( iv_matnr = ls_item-matnr iv_werks = ls_item-werks iv_charg = iv_charg ) = abap_false.
      ev_error = |No packed stock of { ls_item-matnr } with batch { iv_charg } in plant { ls_item-werks }|.
      RETURN.
    ENDIF.

    " ---- the item carries a single batch: replace it (v5) -------------
    ...
    " ZVBAP_BATCH. A list row next to a single batch would make a
    " document VA02 refuses to save, so on such an item the batch given
    " goes INTO the Batch field, in place of the one there.
    IF ls_item-charg IS NOT INITIAL.
      IF ls_item-charg = iv_charg.
        ev_error = |Item { CONV i( iv_posnr ) } already carries batch { iv_charg }|.
        RETURN.
      ENDIF.
      SELECT COUNT( * ) FROM zvbap_batch
        ...
        INTO @DATA(lv_list).
      IF lv_list > 0.
        " Should not exist (VA02 refuses it); report rather than guess.
        ev_error = |Item { CONV i( iv_posnr ) } carries batch { ls_item-charg } AND a batch list - correct the item in VA02 first|.
        RETURN.
      ENDIF.
      ev_error = set_item_batch( iv_vbeln = iv_vbeln iv_posnr = iv_posnr iv_charg = iv_charg ).
      IF ev_error IS NOT INITIAL.
        ev_error = |Batch { iv_charg } could not be put on item { CONV i( iv_posnr ) }: { ev_error }|.
        RETURN.
      ENDIF.
      write_log( iv_vbeln = iv_vbeln iv_posnr = iv_posnr iv_charg = iv_charg
                 iv_action = 'C' iv_user = iv_user
                 iv_note = |was { ls_item-charg } - { ls_item-matnr } plant { ls_item-werks }| ).
    ...
    " ---- the item carries a list, or nothing: add a row ----------------
  ...
  METHOD remove_batch.
    ...
    " Not a list row. The item's single batch (VBAP-CHARG)? Then clear the
    " field - the item names no batch afterwards and a list can be built
    " on it (v5).
    SELECT SINGLE charg, matnr, werks FROM vbap
      WHERE vbeln = @iv_vbeln AND posnr = @iv_posnr
      INTO @DATA(ls_item).
    IF sy-subrc <> 0 OR ls_item-charg IS INITIAL OR ls_item-charg <> iv_charg.
      ev_error = |Batch { iv_charg } is not assigned to item { CONV i( iv_posnr ) }|.
      RETURN.
    ENDIF.

    ev_error = set_item_batch( iv_vbeln = iv_vbeln iv_posnr = iv_posnr iv_charg = space ).
    IF ev_error IS NOT INITIAL.
      ev_error = |Batch { iv_charg } could not be taken off item { CONV i( iv_posnr ) }: { ev_error }|.
      RETURN.
    ENDIF.
    write_log( iv_vbeln = iv_vbeln iv_posnr = iv_posnr iv_charg = iv_charg
               iv_action = 'R' iv_user = iv_user
               iv_note = |single batch cleared - { ls_item-matnr } plant { ls_item-werks }| ).
    CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'
      EXPORTING
        wait = abap_true.
