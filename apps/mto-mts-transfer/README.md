# MTO to MTS Transfer (ZMTOS) — custom Fiori app

Freestyle SAPUI5 worklist app, the clean-core replacement for **ZSOL_MTOS_PROCESS**.
Binds to the OData V4 service of [`backend/mto-mts-transfer-rap`](../../backend/mto-mts-transfer-rap)
(`MtoStock` entity set) and exposes the service's static action(s) as
table actions.
...
- `REPLACE_WITH_MTO_MTS_SERVICE` (manifest.json) → the OData V4 service binding
  name created in ADT for service definition `ZUI_MTO_MTS` (e.g. `ZUI_MTO_MTS_O4`).
